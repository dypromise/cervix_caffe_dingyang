...
import sys
caffe_root = '/mnt/lustre/dingyang/cervix_caffe_dingyang/sensenet/'  
...
def congate_(nparray,textfile,csvfile,num_,offset):
    ...
    for line in lines:
        ...
        arr[image_num-offset] += nparray[index]
        index+=1
    arr/=float(num_)
    ...
    for i in range(int(len(lines)/num_)):
        image_name = str(i+offset)+'.jpg'
...
def doClassify(model_def,model_weights,image_dims,mean_value,test_dir,test_txt, csv_postfix):
    caffe.set_mode_gpu()
    model_def = model_def
    model_weights = model_weights
    predictor = Classifier( model_def, model_weights, image_dims=(375,375), mean=np.array([97.5,91.5,121.2],dtype='float64'))
    inputs = make_inputs(test_dir, test_txt)
    pred_1,pred_2,pred_3 = predictor.predict(inputs,oversample=True)
    offset_ = {0:10000,1:10512,2:11024,3:11536,4:12048,5:12560,6:13072,7:0}
    # congate_(pred_1, test_txt, '/mnt/lustre/dingyang/cccc1_'+str(csv_postfix)+'.csv',4,offset_[int(csv_postfix)])
    # congate_(pred_2, test_txt, '/mnt/lustre/dingyang/cccc2_'+str(csv_postfix)+'.csv',4,offset_[int(csv_postfix)])
    congate_(pred_3, test_txt, '/mnt/lustre/dingyang/cccc3_'+str(csv_postfix)+'.csv',4,offset_[int(csv_postfix)])
    print(pred_1)
    print(pred_2)
    print(pred_3)
...
def main(args): 
    ...
    # model_weights ='/mnt/lustre/dingyang/cervix_caffe_dingyang/myself/inception-v4/savedCaffemodels/iv4_ZXYBN_ADLoss_b1b7jie_36000washed_iter_10300.caffemodel'
    # image_dims=(399,399)
    ...
    test_txt= args[4]
    csv_postfix = args[5]
    doClassify(model_def, model_weights, image_dims, mean_value, test_dir, test_txt, csv_postfix)
